...
for ion in ['H I 1215','C IV','O VI']:
    for i in range(len(machs)):
        ...
        for m in np.arange(0, size1):
            for j in range(64):
                large100.append(col_sort_100[-size1:][m])

        restest = col_sort_800[-size8:][-125696:] - large100[-125696:]
        ax[i].plot(np.log10(restest), y8[-125696:],  label=ion+'(800-100)', color = colors[ion])
        ...
        ax[i].set_xlabel("$\log10(N_{800} - N_{100})$", fontsize = 13)
        ax[i].set_xlim(8, 17)
        #ax[i].set_yscale('log')
...
#ax[0].set_ylim(0, 1)
ax[0].set_ylabel('Cloud Area', fontsize=13)
plt.tight_layout()
